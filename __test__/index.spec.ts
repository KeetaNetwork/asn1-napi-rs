...
test('JS boolean to ASN1 conversion', (t) => {
  t.deepEqual(
    lib.JStoASN1(true).toBER(),
    new Uint8Array([0x1, 0x1, 0xff]).buffer,
  )
  t.deepEqual(
    lib.JStoASN1(false).toBER(),
...
test('JS bit string to ASN1 conversion', (t) => {
  ...
  t.deepEqual(
    lib.JStoASN1(input).toBER(),
    new Uint8Array([0x03, 0x06, 0x00, 0xa, 0x10, 0x14, 0x20, 0x9]).buffer,
  )
})
...
test('ASN1 to Js Date conversion from byte code', (t) => {
  const obj = new lib.Asn1([
    0x18, 0xf, 0x32, 0x30, 0x32, 0x32, 0x30, 0x39, 0x32, 0x36, 0x31, 0x30, 0x30,
    0x30, 0x30, 0x30, 0x5a,
  ])
