...
const TEST_DATES_ASN1 = [
	new Uint8Array([
		0x17, 0x0d, 0x37, 0x30, 0x30, 0x31, 0x30, 0x31, 0x30, 0x30,
		0x30, 0x30, 0x30, 0x30, 0x5a,
	]).buffer,
	new Uint8Array([
		0x17, 0x0d, 0x32, 0x32, 0x30, 0x39, 0x32, 0x36, 0x31, 0x30,
		0x30, 0x30, 0x30, 0x30, 0x5a,
	]).buffer,
	new Uint8Array([
		0x17, 0x0d, 0x32, 0x32, 0x30, 0x39, 0x32, 0x36, 0x31, 0x30,
		0x31, 0x30, 0x33, 0x32, 0x5a,
	]).buffer,
	new Uint8Array([
		0x18, 0x13, 0x32, 0x30, 0x35, 0x32, 0x30, 0x39, 0x32, 0x36,
		0x31, 0x30, 0x31, 0x30, 0x33, 0x32, 0x2e, 0x34, 0x32, 0x30,
		0x5a,
	]).buffer
]
test.only('JS Date to ASN1 conversion', (t) => {
...
test('ASN1 to Js Date conversion from byte code', (t) => {
	TEST_DATES_ASN1.forEach((v, i) => {
		...
		t.deepEqual(obj.intoDate(), TEST_DATES[i].out ?? TEST_DATES[i].in)
